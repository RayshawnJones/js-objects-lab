...
const selectedStarterPokemon = pokemon.find(p => p.starter);
...
const pokemon1 = pokemon.find(p => p.name === "Pikachu");
const pokemon2 = pokemon.find(p => p.name === "Charmander");
const pokemon3 = pokemon.find(p => p.name === "Squirtle");

game.party.push(pokemon1, pokemon2, pokemon3);

//  make use of a compare function to define the sort order of .sort()
/*
Exercise 6
...
const evolvedFormId = evolvedPokemon[starterPokemon.id];
const evolvedForm = pokemon.find(p => p.id === evolvedFormId);
...
// const pokemonToCatch = pokemon.find(p => p.name === "Mewtwo");
